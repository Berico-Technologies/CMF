...
This is a heavily modified version of Data.js, specifically to make the graph model live harmoniously with
LdapJS.
...
(function(){
  ...
  */
  ...
  Data.Query = {
    ...
    query: function(qry) {
      ...
      function match(obj, qry) {
        ...
        _.find(qry, function(value, attribute) {
          var val = attribute === "type" ? obj.types : obj.attributes[attribute];
          if (attribute === "_id") val = obj._id;
  ...
  // collections of attributes that belong to a certain group of objects.
  ...
  Data.Type = function(type) {
      ...
      _.each(this.attributes, _.bind(function(attribute, key) {
        attribute.type = _.isArray(attribute.type) ? attribute.type : [ attribute.type ];
        attribute.unique = _.isBoolean(attribute.unique) ? attribute.unique : true;
      }, this));
  };
  ...
  _.extend(Data.Type.prototype, _.Events, {
    ...
    toJSON: function() {
      return {
        ...
        meta: this.meta,
		extends: (typeof(this.extends) !== "undefined")? this.extends : "/type/type",
        indexes: _.map(this.indexes, function(i) { return i.attributes })
      }
    }
  });
  

  Data.ExtendedType = function(type, graph){
	var dataType = new Data.Type(type);
	var superType = graph.get(type.extends);
	_.extend(dataType.attributes, superType.attributes);
	dataType.extends = type.extends;
	return dataType;
  }
  ...
  // Provides access to attributes, defined on the corresponding `Data.Type`.
  ...
  _.extend(Data.Object.prototype, _.Events, {
    ...
    attribute: function(attribute) {
      ...
      _.find(_.clone(this.types).reverse(), _.bind(function(type) {
        return p = this.host.get(type).attributes[attribute];
      }, this));
      return p;
    },

    
    // There are four different access scenarios for getting a certain attribute
    // 
    // * Unique value types
    ...
    // attribute of a resource is represented as a non-unique `Data.Hash` 
    // of `Data.Node` objects, even if it's a unique attribute. So if you want 
    ...
    get: function(attribute, key) {
      var p = this.attribute(attribute),
	...
	addReference: function(attribute, item) {
		var p = this.attribute(attribute),
	          value = this.attributes[attribute];
	...
	removeReference: function(attribute, item) {
		var p = this.attribute(attribute), value = this.attributes[attribute];
		if (!p || value === undefined || !_.isArray(value)) return false;
		var i = _.indexOf(value, item);
		if (i != -1) { value.splice(i, 1); } else { return false; }
		return true;
	},

    // Sets attributes on the object
    // Existing attributes are overridden / replaced
    set: function(object) {
      var that = this;
      ...
      _.each(object, _.bind(function(value, key) {
        if (!that.attribute(key) || key === "type") return;
        that.attributes[key] = value;
      }, this));
    },

    // Serialize an `Data.Object`'s attributes
    toJSON: function() {
      return _.extend(this.attributes, {_id: this._id, type: this.types})
    }
  });
    ...
    set: function(node) {
      ...
      function createNode() {
        ...
        if((_.last(types) === "/type/type")){
			return typeof(node.extends) === "undefined" ? new Data.Type(node) : new Data.ExtendedType(node, g);
		}
	    return new Data.Object(node, this);
	  }
